...
// set is insufficient: eight mutations of a call site survived one.
//
// This package has no deferred action. All eight of its recommendations were
// classified in Phase 13.1B.
var transportClassification = map[string]classificationExpectation{
	recommendNameNotResolved:          {diagnosis.SafetyVerify, false},
	recommendResolutionFailed:         {diagnosis.SafetyVerify, false},
	recommendConnectionNotEstablished: {diagnosis.SafetyVerify, false},
	recommendEndpointDoesNotSpeakTLS:  {diagnosis.SafetyObserve, false},
	recommendIdentityMismatch:         {diagnosis.SafetyCompare, false},
	recommendChainNotTrusted:          {diagnosis.SafetyCompare, false},
	recommendCertificateNotValidNow:   {diagnosis.SafetyCompare, false},
...
func TestEveryProducedRecommendationCarriesItsFrozenClassification(t *testing.T) {
	assertFrozenClassification(t, everyTransportFinding(t), transportClassification, nil, nil)
}

// everyTransportFinding covers all eight recommendations, which everyFinding does
// not: it exists to prove three codes build, and the five TLS classes share one.
//
// Extended here rather than there, because everyFinding's own assertion — exactly
// three findings — is what makes it useful to its callers.
func everyTransportFinding(t *testing.T) []domain.Finding {
	t.Helper()
	out := everyFinding(t)
	for _, class := range []domain.FailureClass{
		...
		out = append(out, TLS(rctx(requestedTLS(t, tlsFail("10.0.0.1:5432", class))))...)
	}
	return out
}
