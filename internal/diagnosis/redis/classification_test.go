...
// redisClassification is the frozen per-action expectation.
//
// Two actions are absent on purpose: `recommendCredentialWithheld` and
// `recommendCommandNotPermitted` are Phase 13.1C's REC-052 and REC-055, and they
// are asserted unclassified below rather than left unlisted.
...
// redisDeferred are the two actions Phase 13.1C owns, which must stay
// unclassified until that review rewrites them.
var redisDeferred = map[string]string{
	recommendCredentialWithheld:  "REC-052",
	recommendCommandNotPermitted: "REC-055",
}
...
func TestEveryProducedRecommendationCarriesItsFrozenClassification(t *testing.T) {
	assertFrozenClassification(t, everyFindingShapeFindings(t),
		redisClassification, redisDeferred, nil)
}
