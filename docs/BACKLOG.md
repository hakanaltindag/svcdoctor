...
| **Twenty-four recorded evidence attributes are consumed by nothing** | **Measured by Phase 10.7A (ADR 0089 §1.2) and it reframed the roadmap.** Of 59 attributes the four adapters record, 24 are read by no rule and no renderer — Kafka 10 of 14, PostgreSQL 9 of 17, RabbitMQ 4 of 21, Redis 1 of 7. **svcdoctor's diagnostic frontier is consumption, not acquisition**, and no new probe, command or query should be weighed while that number stands. Most of the 24 are protocol bookkeeping or already decided against — Kafka's `controller_id` was measured returning 1,1,2,1,1,3,2,3 on a stable cluster; RabbitMQ's four close attributes are what the adapter derived the failure class from | Each needs **an operator question first**, per ADR 0089 §1.2's inventory. This is the pool the next several candidates should be drawn from, not a protocol wish-list |
