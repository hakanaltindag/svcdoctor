...
| **`BasisBuilder.Miss`, `.Block` and `.Contradict` have no production producer** | Phase 10.0 froze four evidence relations; Phase 10.4A measured that only `.Support` is ever called, from **one** site. `.Miss` is the one that matters here — its own doc comment calls it *"the input to a next-evidence recommendation"* — so today no test can check that a discriminator corresponds to a genuinely missing observation. **Phase 10.4B deliberately does not force a producer**: manufacturing one to justify a primitive is the inversion ADR 0054 forbids | A rule that genuinely records one, in the phase that needs it |
