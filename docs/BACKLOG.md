...
| **Phase 12.1B — Kubernetes client, authentication and acquisition** | **CLOSED — IMPLEMENTED, uncommitted. The first Kubernetes production code.** `client-go`/`api`/`apimachinery` **v0.36.4** pinned, importable by `internal/adapter/kubernetes/client` alone behind the **nine-package allowlist**, enforced by `depguard` **and** by an independent AST guard. Measured, not estimated: modules **2 → 40**, `go.sum` **4 → 117** lines, binary **10,310,258 B → 38,494,066 B** (×3.73), linked packages **219 → 518**, `os/exec` and `net/http` newly linked; the minor was chosen by measurement — v0.36.4 is 39 modules / 31.2 MB against v0.37.0's 47 / 32.4 MB. **`client-go/discovery` and `plugin/pkg/client/auth/exec` are *linked* but neither is imported and no discovery request is made** — proven against a request-counting server, because a linker check cannot say it; **no auth *provider* package is linked at all**, so `auth-provider` fails closed twice. **The exec sentinel test passes**: an exec kubeconfig whose command is the test binary itself is refused at `Inspect`, `Connect` **and** `Acquire`, the sentinel file does not exist, and the API server counted **zero** requests. `clientcmd` **parses only** — the `rest.Config` is built by hand, so `ExecProvider`/`AuthProvider` are never populated on any path. Four auth modes all exercised end to end; **one `Reveal` and one `SecretFor`**, both **4 → 5**, the first outside a `wire` package because client-go owns the transport. Three bounded reads, server-side filtered, **exactly 3 requests nominal and exactly 17 worst-case**; every stop reason exercised; **incomplete never becomes zero**; `410` never restarted; cancellation issues no further round trip. Five evidence nodes, one subject, **no per-Pod or per-endpoint node** — a 4,000-backend Service produces the same five. **Zero findings, zero rules, zero renderer files changed, no new CLI command.** Counts unchanged: `SchemaVersion` **1**, `RunSchemaVersion` **1**, codes **65**, rules **22**, failure classes **42**, `RuleContext` **3**, exit codes **5**, and **not one closed vocabulary gained a member** | **Mutation 41 planted / 41 caught / 0 survivors**; fuzz 5 targets × 45 s; `make check` green. **The first mutation run caught 34 of 41, and every one of the seven survivors was a real gap in a guard** — a loopback fixture that cannot see an inherited proxy, an empty selector map that `json:",omitempty"` cannot put on the wire, a cancellation whose outcome arrives by two routes, and a `Status.Message` branch unreachable for the inputs tested. Each is recorded with its fix in `docs/validation/PHASE121B_KUBERNETES_CLIENT_AUTH_ACQUISITION.md` §10.4. **Next: Phase 12.1C — the two rules, F1–F4, the CLI case and the golden reports**, then 12.1D for `kind` |
| **Three recorded narrowings against the frozen Kubernetes contract, two corrected counts and one fixed drift** | **Found by Phase 12.1B while implementing ADR 0094.** **(1) In-cluster derives `kubernetes.default.svc:443`, not `KUBERNETES_SERVICE_HOST`** — the variable cannot be read at decode without breaking `TestOnlyTheResolverReadsTheEnvironment` and ADR 0071 §8.3, and a configuration must be checkable on a laptop. The API Service's own name is true in every cluster, is what TLS verifies, and the connection still goes to the injected address: ADR 0028's logical-endpoint rule, unchanged. **(2) `Decode` reads the kubeconfig**, so an `exec` stanza is a configuration error at **exit 2** with nothing dialled rather than one target's execution failure at **exit 4** — the Phase 9.1C defect, avoided. `config.Factory`'s *"none of them performs I/O"* was **amended rather than bent**; `internal/fleet/config` still opens exactly one file. **(3) "Associated by label alone" is not recorded**, because §11.1's frozen table has no key for it and adding an eighteenth would be inventing one — such slices are still included, as §7.3 requires. **(4) Two counts in the frozen records were clerical and are corrected in place, with the originals preserved** — §11.1's attribute table lists **seventeen** rows each naming a consumer while its heading said fifteen and its closing sentence and ADR 0094 §2.9 said sixteen; and §4.4's import table has **nine rows naming ten paths**, one row admitting both `k8s.io/api` object groups, which is where *"nine-package allowlist"* came from. **Neither set changed.** **(5) One real drift was found by the review pass and fixed**: `k8s.namespace` and `k8s.service_name` were implemented on `k8s.target` and the frozen table puts them on `k8s.service`. They moved; the anchor now carries no attributes, exactly as `target.requested` does not (ADR 0042 §6); the keys, kinds and values are unchanged | Each is an implementation reading of a gap, or a clerical correction — **no frozen decision, set or rule was rewritten**, and ADR 0094's contract is unmodified. Phase 12.1C decides whether the label-alone disclosure earns a key |
| **Cross-page `continue` consistency — VERIFIED, and the answer was stronger than the assumption** | **Closed by Phase 12.1B.** Phase 12.1A could not obtain this and designed conservatively around not knowing. Official sources say a paginated list **is** a consistent snapshot — *"the `resourceVersion` of the collection remains constant across each request"* — that a token must be used *"with identical query parameters (except for the value of continue)"*, and that expiry (*"generally five to fifteen minutes"*) is answered with *"a 410 ResourceExpired error"* because *"continuing a consistent list may not be possible"*. **No behaviour changed**: svcdoctor already sent identical parameters (now asserted), already refused to restart on a `410`, and already relied on completeness rather than atomicity. What changed is the reasoning — a `410` means *this snapshot cannot be continued*, so a restart would present two snapshots under one completeness flag | The guarantee is scoped to **one list operation** and says nothing across the three, which are three separate moments. That is why no claim spans them, and why a finding is scoped *"at the time of these API observations"* |
| **A Kubernetes target exits 0 whatever it observed** | **Known and deliberate in Phase 12.1B.** No rule is wired, so an unreachable API server, a missing Service and a denied read all produce `SummaryStatus OK` and exit 0. It is the phase split working as ADR 0094 §2.12 designed it — 12.1C's diff is the entire behavioural change — and the README says *"no diagnosis yet"* rather than implying otherwise | **Phase 12.1C.** The four finding codes and two rules arrive together with the exit codes they move |
| **Phase 12.1A — Kubernetes adapter contract freeze** | **CLOSED — CONTRACT FROZEN, zero open blockers. Documentation only; no Go, no test, no config, no fixture, no `go.mod`/`go.sum` change. ADR 0094.** Nine contracts closed. **B1 (exec auth) closed by measurement**: a kubeconfig whose exec plugin writes a sentinel file executed **nothing** at `clientcmd.Load`, **nothing** at `ClientConfig()`, **nothing** at `NewForConfig()`, and ran **only on the first API request** — so refusing after parse and before any request is **structural**, not a promise, and every dangerous field (`Exec`, `AuthProvider`+`cmd-path`, `Impersonate`, `TokenFile`, `ClientKey`, `ProxyURL`) is already visible as a typed value at that point. `auth-provider` **fails closed for free** — `NewForConfig` errors with *“no Auth Provider found”* unless a `plugin/pkg/client/auth/**` package is imported. Impersonation and `proxy-url` propagate into `rest.Config` **with no error**, which is why both are refused rather than reported. **B2 (dependency) closed by authorization with a recorded cost**: **2 → 46–47** modules, **10,310,258 B → ~36 MB**, `os/exec` and `net/http` linked where neither is today; nine-package import allowlist; the full `Clientset`, dynamic, discovery, informers, listers, cache, watch, controller-runtime and kubectl **refused** as a capability surface. **Four findings** — `SERVICE_NOT_FOUND`, `API_ACCESS_DENIED` (acquisition) and `SELECTS_NO_PODS`, `NO_READY_ENDPOINT` (semantic) — all CONFIRMED / `AuthorityDirect` / HIGH, no discriminator; `401` earns **no** code. Counts for the implementing phases: codes **65 → 69**, rules **22 → 24**, `Reveal`/`SecretFor` **4 → 5**; `SchemaVersion` **1**, failure classes **42**, and **no closed vocabulary gains a member** | **KAC-001 … KAC-040.** Frozen: exec/auth-provider/impersonation/`proxy-url`/`insecure-skip-tls-verify` **refused before any network operation** (exit 2), with a sentinel-file negative test as a release gate; four auth modes (token, token file, client cert+key, in-cluster); **explicit** kubeconfig path, context and namespace with **no fallback of any kind**; RBAC **3 resources, 2 verbs, 1 namespace**, and *a label selector narrows the request, not the grant*; three API operations, server-side filtered, sequential; budgets limit **500** / **8** pages / **4,000** Pods / **256** slices / **10,000** endpoints; **a budget, a remaining `continue`, a `410` or a cancellation makes a set INCOMPLETE and never zero**; `ready` nil ⇒ **true**, `serving` nil ⇒ **true**, `terminating` nil ⇒ **false**, and effective-ready reads **`ready` alone**; association by `kubernetes.io/service-name` **plus an owner-UID check** (no fourth API call); **publishes, never reachable**. **Next: Phase 12.1B — Kubernetes client, authentication and acquisition** (no finding, no rule, no CLI command; byte-identical reports for the four existing services) |
| **Two contract narrowings against ADR 0093, both recorded rather than smoothed away** | **Found by Phase 12.1A.** **(1) Pod state is not retained at all**, where ADR 0093 said *observation-only*: **no admitted finding consumes a single Pod field**, so MVP-D keeps no Pod name, UID, label, phase, readiness, container status, restart count, image, node name or IP — the data-minimization rule ADR 0090 §7 states, applied honestly. It also **dissolves ADR 0093's open renderer question**: without per-Pod and per-endpoint nodes the report is a single-path journey and `terminal.serviceView` needs **no change at all**. **(2) Client certificates are an admitted authentication mode**, which ADR 0093 did not anticipate — **forced by the fixture strategy, since `kind` and `kubeadm` kubeconfigs authenticate with `client-certificate-data`/`client-key-data`**; refusing them would have made the real-cluster validation contract unachievable | **ADR 0072 §14 condition 1 has fired and is reopened narrowly**: the private key is secret material carried by `security.Secret`, the certificate and CA bundle are public and are not. §13's refusal of an `exec:` provider is **upheld, not reopened**. The consequence is one production `Reveal` site for Kubernetes and a **4 → 5** count that preserves the one-per-service invariant |
| **`test/security/dependency_test.go`'s stated principle is contradicted by client-go** | **Found by Phase 12.1A; recorded, not fixed — the phase changes no test.** The guard's comment reads *“A transitive dependency appearing under one of them would be a separate finding and is prevented in the only durable way, **by choosing dependencies that have none**.”* Both current modules have **zero** transitive dependencies; client-go has **45**. The guard also states why the count matters — *“svcdoctor transmits credentials, so every module in the build graph runs in the same process as a plaintext password”* — which makes this a security-posture change, not bookkeeping | **Phase 12.1B must amend the reasoning, not merely the number**, and say in the same change that the principle now reads *prefer dependencies with none; where that is impossible the exception is recorded in an ADR*. Editing the count and leaving the sentence would make the guard lie |
| **Kubernetes API response byte size is bounded by object count, not by bytes** | **Found by Phase 12.1A; accepted as a known limitation rather than a blocker.** A list `limit` bounds the **number of objects** in a page, not the response's **byte size**: a single Pod carrying large annotations can be hundreds of kilobytes, and 500 of them is not a byte bound. **client-go's typed clients expose no per-response body ceiling**, so obtaining one means replacing the transport. Exposure is bounded in practice by the page size and by svcdoctor retaining **sixteen fields** and discarding every decoded object immediately | Phase 12.1B records it in `docs/SECURITY.md` as a known limit. A byte ceiling needs a transport svcdoctor controls, or a measured case where the object bound proved insufficient |
| **Cross-page `continue` consistency could not be verified from official documentation** | **Found by Phase 12.1A.** The `kubernetes.io` API-concepts page truncated before *“Retrieving large results sets in chunks”* on three separate fetches, so whether a paginated list is a consistent snapshot across pages is **NOT WEB-VERIFIED**. The contract therefore designs conservatively: svcdoctor relies on **completeness** — every page followed, none remaining, within budget — and **never on cross-page atomicity**, which is the safe direction whatever the guarantee turns out to be | Phase 12.1B re-verifies from official documentation **before relying on anything stronger**. Nothing in the frozen contract depends on the answer |
| **Phase 12.0 — Kubernetes diagnostic adapter architecture and scope audit** | **CLOSED — SELECT ARCH A, MVP-D. Documentation only; no Go, no test, no config, no fixture, no dependency. ADR 0093.** Weighed **19** object kinds and **18** finding candidates: **4 admitted, 5 deferred, 9 rejected**. The decision turned on one web-verified fact — **Kubernetes's own API reference documents `ContainerStateWaiting.reason` and `ContainerStateTerminated.reason` as free-form strings**, so `OOMKilled`, `CrashLoopBackOff` and `ImagePullBackOff` are kubelet and container-runtime conventions rather than API contract, and `CrashLoopBackOff` is not even a Pod phase. Every attractive Pod-level finding keys on one of them, and what survives the free-text prohibition (the rule ADR 0066 and ADR 0069 §8 already apply to Redis and RabbitMQ) is one column of `kubectl get pods`. **The one Kubernetes reason vocabulary that *is* a contract enum is `metav1.StatusReason`**, which is why two of the four admitted findings are about API status and none is about a container. **What survives is backend publication**: answering *“what does Kubernetes currently publish as ready for this Service?”* needs zero free text and requires six steps `kubectl` does not perform — slices are found by the `kubernetes.io/service-name` label, multiple slices per Service are normal and dual-stack guarantees ≥2, **`ready: nil` means true**, `serving`/`terminating` have their own nil defaults, endpoints legitimately appear in more than one slice, and the enumeration must be proven complete. **MVP-D**: one `namespace + Service` target, 3 object kinds, **4** finding codes, **2** rules, Pod state as **observation only**, and **no schema, graph or `RuleContext` change**. Frozen counts unchanged: 65 / 22 / 42 / 3 / 2 / 4 / 4 / 5 / 1 / 1 | **KDA-001 … KDA-030.** Frozen: the nine-class authority model (**API state never becomes hidden runtime cause**); **publication, never reachability**; **a budget or page limit becomes INCOMPLETE, never “zero”** (Kafka 10.2's rule, unchanged); reads are not a snapshot and controller lag is never misconfiguration; deterministic ordering under API list reordering; subject `<kind>/<namespace>/<name>` with **UID as internal correlation only**; **Kubernetes credentials reach the API server and nothing else** — structural via ADR 0028's `security.Endpoint` binding; **no active probing of Pods, ClusterIPs, Service DNS or endpoints**; and **OUT**: Events, logs, metrics, a NetworkPolicy solver, CNI/eBPF, service mesh, Ingress/Gateway, Node lists, Secrets, ConfigMap contents, annotations and label dumps. **Next: Phase 12.1A — Kubernetes Adapter Contract Freeze** (documentation only), which must close both blockers below before any Go |
| **Blocker B1 — kubeconfig `exec:` credential plugins collide with a permanent decision** | **CLOSED by Phase 12.1A / ADR 0094 §2.2 — REFUSED, and the refusal point is measured: client-go runs the plugin only on the **first API request**, so refusing after `clientcmd.Load` and before any request is structural. ADR 0072 §13 upheld, not reopened.** **Measured by Phase 12.0, not suspected.** ADR 0072 §13 refuses an `exec:` / command provider — *“Arbitrary code execution driven by a config file. It is the single largest surface any of these would add”* — with reopen condition **“None. This is a decision, not a deferral.”** A kubeconfig `exec:` stanza is exactly that shape, and **EKS, GKE and AKS generate one by default**. Measured in a temporary module outside the repository: `k8s.io/client-go` v0.37.0 links **`os/exec`** and `k8s.io/client-go/plugin/pkg/client/auth/exec` — and **avoiding `clientcmd` does not help**, because the exec provider is reachable from `rest` itself. **No production file imports `os/exec` today**, and four guards say so: `test/security/fleet_boundary_test.go` (*“there is no exec credential provider (ADR 0072 §13)”*), `test/security/diagnosticcore_test.go`, the `internal/cli` depguard rule (*“the command runs no subprocesses”*), and `internal/app/boundary_test.go`. So *“refuse exec by not linking it”* is **not available**; the guarantee can only be behavioural | **Phase 12.1A must decide**, and may not default. Either **(B+D)** refuse `exec:` stanzas at parse time and materialize credentials externally — a token by `env` or `file` plus a CA bundle, which is **compatible with ADR 0072 §13 unchanged** because those are the two sources §2 already supports, and is the in-cluster shape — **or** reopen ADR 0072 §13 **with its own security review**. Phase 12.0 recommends B+D and decides neither |
| **Blocker B2 — the Kubernetes client library is the largest supply-chain change ever proposed here** | **CLOSED by Phase 12.1A / ADR 0094 §2.1 — CLIENT_GO_AUTHORIZED behind a nine-package allowlist, with the cost recorded rather than minimized.** **Measured by Phase 12.0.** svcdoctor today: **2** external modules, a **4-line** `go.sum`, a **10,310,258-byte** binary over 219 packages, and **neither `os/exec` nor `net/http` linked**. A sample program using `k8s.io/client-go` v0.37.0 typed clients: **46–47** module requirements and a **~36.5 MB** binary, linking both. Raw REST is **refused** — it re-creates kubeconfig parsing, TLS configuration, auth providers, pagination and API decoding in the package that talks to a control plane, and *“lightweight” is not a security argument*. `TestTheDependencyCountIsExact` exists to force someone to record this number | **Phase 12.1A decides** the import surface (restricting it to typed `CoreV1` + `DiscoveryV1` plus `rest` is the candidate), the pinned Kubernetes minor, and records the measured module count and binary size rather than discovering them later |
| **A Kubernetes target is not endpoint-shaped, and the fleet contract requires one** | **CLOSED by Phase 12.1A / ADR 0094 §2.10: the factory declares `DefaultPort() 443` and the target needs no `host` field, because the API server host is **derived** during decode from the context the target already names. The generic contract is satisfied with no widening, and the Kubernetes target is a fleet citizen from day one.** **Found by Phase 12.0.** `internal/fleet/config.Factory` requires **`DefaultPort() uint16`**, `NewRegistry` **rejects a zero default port**, `Common` carries `Host`/`Port`/`TLS`/`Credentials`, and `load.go:232` makes a host **required**. A Kubernetes target declares a namespace and a Service, not a host and a port. The consequence is that the first scope is a **leaf CLI command only** and is **not registered with the fleet**, which is a real asymmetry — Redis and RabbitMQ both shipped as leaf commands before `run --config` existed, so the sequencing has precedent, but the gap does not close itself | Either a `config.Factory` shape that admits a service which is **not endpoint-shaped**, or a defensible derivation of `Host`/`Port` from the resolved API server. A Phase 12.1A question, and not one an implementation phase may answer by widening `Common` quietly |
| **The terminal renderer models a linear journey with one repeated child level** | **DISSOLVED by Phase 12.1A / ADR 0094 §2.9: with no per-Pod and no per-endpoint evidence node, the Kubernetes report is a single-path journey and `serviceView` needs **no change at all**. The problem existed only while Pod objects were retained.** **Found by Phase 12.0.** `internal/render/terminal.serviceView` is *“a table, not a branch”* (ADR 0052 §5) and that half fits — but it models a **per-path journey** (`journey []domain.Step`, `narrowingSteps`) plus exactly **one** repeated child level (`advertisementStep` + `advertisedJourney`). A Kubernetes report has **no journey** and **two** child kinds: selected Pods and published endpoints | **Preferred and free:** use the single child level for one and the existing `observations` mechanism for the other. **The alternative** — `serviceView` gains a second child level — is a renderer change that must be decided in Phase 12.1A and must not happen by accident |
| **Phase 11.0 — diagnostic planning / next-best-evidence opportunity audit** | **CLOSED — DEFER DIAGNOSTIC PLANNING. Documentation only; no Go, no test, no fixture, no harness changed. ADR 0092.** Inventoried **67** frontier cases covering all **65** finding codes and every non-finding state (`UNKNOWN`, `SKIPPED`, graph-blocked, budget-cut, cancelled, and every observation that is deliberately not a finding), and wrote **29** candidate case files — **0 admitted, 9 deferred, 20 rejected**; observation classes `O0` 4 / `O1` 1 / `O2` 2 / `O3` 3 / `O4` 5 / `O5` 2 / `O6` 3 / `O7` **0** / `O8` 1 / `O9` 8; information gain **0 DECISIVE**, 12 MATERIAL, 9 LOW, 8 NONE. **The reason is not that svcdoctor has no epistemic boundaries** — it has many — but that wherever two genuinely competing explanations remain and a discriminating observation exists, one of three things holds: the observation is **not svcdoctor's to take** (intent, privileged configuration, or a network position it does not occupy), svcdoctor **already names it** as a `Discriminator` plus a classified `NEXT_EVIDENCE` recommendation, or taking it would **attack the endpoint, guess, or exceed a bound**. A planner needs *“I cannot distinguish A from B; observation O would distinguish them, **and I could take O**”*; the tree has the first two clauses in shipped findings and **no instance of the third**. **Model A is the minimum justified iteration model and it shipped in Phase 10.4B**, so the deferral costs an operator nothing. Both nominated candidates failed differently: **alternate vantage** has the best epistemics in the audit and fails **8 of 25 gates** (one `VantageSource`, one vantage per report, no execution channel, no two-vantage report shape); the **PostgreSQL admission contrast** passes **all 25** and is still not selected, because selecting it would authorize building what already exists. Frozen counts unchanged: 65 / 22 / 42 / 3 / 2 / 4 / 4 / 5 / 1 / 1 | **DPA-001 … DPA-024.** Frozen: the seven-condition definition with eight excluded shapes; the five-part competing-pair test with four permanently invalid pairs; observation classes `O0`–`O9`; ordinal gain with no score anywhere; the **vantage ceiling** (a two-position contrast proves *behaviour differs by network position* and **never** firewall, route, security group, NetworkPolicy, proxy or server-side); **intent is a premise, never `SelfCollectable: true`**; determinism with **no LLM deciding canonical output**; and the 25 admission gates. **Refused while nothing is admitted:** a `Planner`, a planning loop, `--investigate`, a `DiscriminatorID`, a hypothesis-set engine, an observation scheduler, a planning budget, and any producer for `CONTRADICTION`/`MISSING`/`BLOCKED`. **Reopens only when** a production diagnosis leaves two competing explanations **and** an observation exists that svcdoctor could take **from its own vantage with its existing credential authority**, is class `O0`/`O1`/`O2` with a bounded response, and would **materially** change what svcdoctor may claim (ADR 0092 §7) |
| **`docs/FINDINGS.md` has no entry for any of the nine `REDIS_*` finding codes** | **Found by Phase 11.0; recorded, not fixed — the phase changes no document outside its own record.** The catalog documents Kafka, PostgreSQL, generic transport and RabbitMQ, and contains the word “Redis” exactly once — in a sentence about a *RabbitMQ* code. **No behaviour is wrong and no claim is unsafe**: the nine codes are specified in ADRs 0063–0066, implemented, and covered by tests including `TestReplicaRoleProducesNoFinding` and `TestNoRedisFindingAssertsAnExpectation`. What is missing is the §6-style service section every other service has, and with it the per-code owner step, kind, severity and vantage-dependence table a reader uses to check a report | A documentation change, in its own commit. It is **not** an audit's work and was deliberately not bundled into Phase 11.0 |
| **A received TLS alert is not a typed error, so three declared failure classes stay unproducible** | **Measured by Phase 11.0 on Go 1.26.6** (a scratch program run outside the repository; nothing in the tree changed). A peer's fatal alert — 40 `handshake_failure`, 70 `protocol_version`, 116 `certificate_required`, 48 `unknown_ca` — arrives as **`*tls.permanentError`**, and `errors.As` against `tls.AlertError`, `tls.RecordHeaderError` and `*tls.CertificateVerificationError` returns **false** for all four. The description survives **only in the error text**, which `classifyHandshakeError` refuses by policy — *“Error text is never matched: it differs by platform and Go release”*. So **`TLS_VERSION_MISMATCH`, `TLS_CLIENT_CERTIFICATE_REQUIRED` and `TLS_CLIENT_CERTIFICATE_REJECTED` remain declared with no producer**, and the probe's own comment — which said this of `protocol_version` alone — is right and narrower than the truth | A Go release that exposes a **received** alert as a typed error. Then the three classes become **Class 1** at zero cost and the question is an evidence one, not a planning one. **Re-measure before assuming; never read the text** |
| **Declared operational intent (expected role, expected posture)** | **Opened by Phase 10.3 and deferred by an Accepted record.** Useful diagnoses need to know what the operator expected — that an endpoint should be a primary, that TLS should be required, that a vhost should be reachable — and without it svcdoctor can report an **observed** property and never "the wrong one". ADR 0083 §2.6 froze it out of the whole of Phase 10; ADR 0085 §5 records the archaeology that confirms nothing carries it today: not the CLI, not the target YAML, not the adapter parameters, not `domain.Report`. Classified `CONFIG_CONTRACT_PRESSURE` + `ADR_DECISION_REQUIRED`. **Deferred, not declined** — PostgreSQL role *observation* is implemented as a renderer observation line, and role *mismatch* is not implemented at all | A phase that lands a **small closed vocabulary** of typed expectations — not a policy language — against ADR 0071's strict-schema contract, with its own record. It applies to every service the instant it exists, so never as a side effect of a service phase (ADR 0083 §2.6, ADR 0085 §5) |
| **Thirty-eight recorded evidence attributes are consumed by nothing — and that is not debt** | **Re-measured and re-framed by Phase 10.8A (ADR 0090 §3, §7); the pool is now exhausted, not open.** Of **70** recorded attributes — 59 service, 11 generic DNS/TLS — **10** are read by a rule, **24** by a renderer, **2** by both and **38** by neither: Kafka 12/14, PostgreSQL 11/17, RabbitMQ 4/21, Redis 1/7, generic 10/11. **ADR 0089 §1.2's figures were wrong and are corrected**: its table said 24 while its own prose named 27, Phase 10.7B removed `default_transaction_read_only`, and `postgres.role` and `postgres.server_version` were never in the list. **Every one of the 38 received an individual case file** (`docs/validation/PHASE108A_EXISTING_EVIDENCE_CONSUMPTION_AUDIT.md` §7) — **1 admitted, 6 deferred, 31 rejected**: 14 duplicates of a subject reference or of operator input echoed back, 6 bookkeeping, 4 unsafe to consume, 7 of no operator value. **ADR 0090 §7 freezes the principle**: an attribute earns canonical evidence for auditability, protocol legibility, machine consumption or a future tripwire, and that earns it **no terminal line, no finding and no roadmap item**. This row is therefore no longer a candidate pool | **Closed as a pool.** Only six named candidates remain, each with its own condition below; a rejected verdict is reopened by **an operator question**, never by the observation that a field exists (ADR 0090 §12.7) |
| **Phase 10.8A — existing evidence consumption audit** | **CLOSED — documentation only; no Go, no test, no fixture, no harness changed.** Selected **one** winner from 38 candidates: **consume `rabbitmq.close_outcome`** so a capacity-ceiling refusal names *which* ceiling — node, virtual host or user. Today all three collapse into `RABBITMQ_CONNECTION_NOT_PERMITTED`, whose detail hedges *"**Where** the endpoint named a capacity ceiling…"* and whose recommendation says *"review any **node, virtual host or user** connection limits"* — while the cited node holds a byte-equality-verified constant naming exactly which. **ADR 0069 §6 already decided this** (*"the `FindingCode` and the sentinel attribute explain which ceiling"*) and §9.4 assumed the operator-visible difference was preserved; §8 permits it by exclusion, since all three ceilings satisfy `namedConditions`' live-measurement membership rule. **Corrected 2026-09-06 by Phase 10.8A.1 / ADR 0091** on three points of fact — see the row below. Class 1 **canonical explanation enrichment**, not a new diagnosis: the diagnosis `RESOURCE_LIMIT_REACHED` already exists and is already correct, and the defect is information loss between canonical evidence and operator prose. **Acquires nothing, adds no diagnostic rule, no inference, no `FindingCode` and no confidence admission** | **ECA-020 … ECA-024.** Closed constant set relocated to `internal/service/rabbitmq`; an unmapped outcome renders today's text byte for byte; **no `FindingCode`, severity, confidence, exit code, failure class, schema, `RuleContext`, dependency or CLI change**; and **a real fixture must produce all three ceilings** at `limit 0` plus a LavinMQ contrast proving unchanged output. 10.8B may not close without both |
| **Phase 10.8A.1 — canonical finding explanation contract correction** | **CLOSED — documentation only; no Go, no test, no fixture, no harness changed. ADR 0091, superseding ADR 0090 §5, §6 and §11 in part.** Phase 10.8B **stopped before implementation** and was right to: `Finding.Detail` and `Finding.Recommendations` are **canonical domain fields** declared in `findingJSON` and emitted by `Finding.MarshalJSON`, so ADR 0090 §6's *"additive in presentation and inert everywhere else"* is false and its triple — presentation activation + enrich Detail + canonical JSON unchanged — is **internally inconsistent**. No location resolves it: `writeFindings` forbids generating prose (*"a renderer rewording them would be making a diagnostic claim in a presentation layer"*) and the Result-block mechanism reads the graph, not the finding's cited evidence. **svcdoctor has no presentation layer for finding prose.** The fix is the distinction 0090 never drew: **schema change vs value change** — field set and both schema versions unchanged, `detail` *value* more specific, **canonical JSON bytes move for mapped outcomes and stay byte-identical for every other**. Frozen term: **canonical explanation enrichment**. Candidate **not** reclassified — still C1-A. Two factual corrections, both reducing work: **LavinMQ does produce `VHOST_CONNECTION_LIMIT`** (template `L3`, measured by `LMQ-06` on 2.3.0), and that outcome is **already fixture-proven on real RabbitMQ across all three versions (`RAB-21`) and on real LavinMQ** | **CFE-001 … CFE-014.** For 10.8B: canonical JSON byte change on mapped outcomes is **no longer a stop condition — it is expected**; byte-identity for unmapped outcomes must be **proven by test**; **recommendations stay byte-identical by default** (ADR 0091 §7) and changing one carries its own burden; the claim ceiling is *the peer **named** a scope while refusing **this attempt*** — never *exhausted* — per the PostgreSQL `53300` precedent; explanation is driven by the **authoritative closed outcome, never product identity** (no `if RabbitMQ`); all four ADR 0081 §2.2b convergence shapes proven; and the remaining real-fixture gap is **`NODE_CONNECTION_LIMIT` and `USER_CONNECTION_LIMIT` only**, with **verified** provisioning rather than `\|\| true` |
| **Phase 10.8B — RabbitMQ/LavinMQ capacity-scope canonical explanation enrichment** | **CLOSED — implemented and measured under ADR 0091.** `RABBITMQ_CONNECTION_NOT_PERMITTED` now names which ceiling the endpoint named — *"The endpoint named a connection limit scoped to the node / virtual host / user."* — replacing the hedge *"Where the endpoint named a capacity ceiling…"* and the recommendation that told the operator to review all three. **No new diagnosis**: no `FindingCode`, rule, confidence, failure class or schema; counts stay 65 / 22 / 42 / 3 / 2 / 4 / 4 / 5 / 1 / 1. The `CloseOutcome` type and its **seven** literals moved to `internal/service/rabbitmq` with type and const **aliases** in `wire`, so there is one authoritative spelling, byte-identical values and zero call-site churn. Selection is a **map lookup on exact keys** double-gated on `RESOURCE_LIMIT_REACHED`; recommendations are byte-identical; unmapped and non-capacity outcomes render today's text exactly. **Canonical JSON bytes change for mapped outcomes and only in `detail`** — expected under ADR 0091 §4, and proven field-by-field | **RCCE-001 … RCCE-023.** Real proof: **RAB-26** node ceiling (new broker `svcd-rabbit-nodelimit`, `connection_max = 0`, TLS), **RAB-27** user ceiling (`set_user_limits`, **verified** provisioning), **RAB-21** vhost ceiling, **LMQ-06** the same explanation on real LavinMQ — a **positive** compatibility proof, since the explanation is earned by the shared closed outcome and never by product identity — and **LMQ-09** that an unmapped LavinMQ refusal gains no scope. Mutations **22/22, 0 survivors**; fuzz **10,317,989 execs, 0 failures**. Two RabbitMQ scenarios fail environmentally (`RAB-16` container restart, `RAB-25` no `[::1]` publishing) and were **proven pre-existing at `5bd6589` via a baseline worktree** |
| **Renderer sanitization of unbounded observation values** | **Now blocks three candidates across two services, not one (ADR 0090 §8.1).** `kafka.sasl.offered_mechanisms` is the **highest-value** unconsumed attribute in the tree — `KAFKA_AUTH_MECHANISM_NOT_OFFERED` tells the operator to go read `sasl.enabled.mechanisms` while the answer is on the node it cites — and mechanism names are copied verbatim off the wire with no allowlist, no length bound and no character restriction (`saslhandshake.go:78-80`, `postgres/wire/authrequest.go:145`). `postgres.sasl_mechanisms` and `postgres.server_version` fail the same gate. **Redis and RabbitMQ already render a verbatim peer version and PostgreSQL is refused one**, which is not a defensible steady state | **One decision for all four services**, with its own record and its own review. Never inside a service phase, and never by bounding one service's list with a local allowlist — that is the conditional sprawl the architecture rule forbids |
| **A product-wide security-posture finding class** | **Named by Phase 10.8A as the home three refused claims are waiting for (ADR 0090 §8.2).** `postgres.scram_iterations` is the strongest C1-B candidate in the audit: ADR 0038 §16 recorded it as *"the only fact that can state a weak server configuration"*, ADR 0038 §17 gave it *"a named future consumer"*, `internal/sasl/scram/parse.go:293` says *"a rule can say so later"*, **Phase 4.6 never built one**, and **ADR 0040 §5 explicitly does not foreclose** *"security posture, weak-mechanism policy, certificate posture, compliance signals"*. Deferred because it answers **no incident question** — connectivity is unaffected and the remedy is a scheduled rotation — and would reopen both ADR 0040 §22's four-key attribute allowlist and the PostgreSQL BASIC freeze | A record that decides whether a posture weakness is a target-side problem worth exit code 1, and answers the Kafka asymmetry: Kafka SCRAM carries the same iteration count and svcdoctor **records it nowhere** |
| **Kafka topology completeness cannot see an unrepresentable advertisement** | **Found by Phase 10.8A (ADR 0090 §9, C2) and not fixed.** ADR 0084 §4 computes `complete` over an exchange's **children** (`topology.go:347`), and an entry whose text cannot be a `Subject` reference produces **no child** (`metadata.go:350`) — so `detailTopologyComplete` can state that *"the counts above account for the whole advertised set"* over a set that silently lost a member. **ADR 0035 §1 anticipated the missing *finding*, not this later *overclaim*** built on top of it in Phase 10.2. The condition needs a control character, invalid UTF-8 or leading whitespace in `advertised.listeners`; an empty host yields the representable ref `:9092` and becomes a `_UNUSABLE` node instead | A supported fixture that produces one — `KAFKA_PHASE3_VALIDATION.md:133` records that **no real run ever has** — or an argued decision that `complete` must read `kafka.metadata.unrepresentable_entry_count` regardless |
| **Two attribute doc comments contradict the code** | **Found by Phase 10.8A (ADR 0090 §9, C3/C4); recorded, not patched, because the phase was documentation-only and no behaviour is wrong.** `internal/service/redis/vocabulary.go:105-106` says *"the renderer reads this one"* of `redis.auth_required` — **no renderer ever has**, since `aba0d45`; the Redis observations table holds `server`, `server_version`, `proto`, `mode`, `role` and nothing else. `internal/service/rabbitmq/vocabulary.go:152` says *"Rules read it to state what the endpoint named"* of `close_outcome` — rules read the derived `FailureClass` at `connectionopen.go:133` | A maintenance change. Deliberately **not** bundled into 10.8B, whose scope is one closed map; C4 is superseded in substance if 10.8B lands |
| **Phase 10.7B — PostgreSQL session read-only observation completion** | **CLOSED — implemented and measured.** 12/12 mutations caught, `default_transaction_read_only = on` proven against a real PostgreSQL 18 server (`svcd-pg-readonly`, started with `-c`, executing no SQL), and `docs/OUTPUT.md` now states the boundary for both session observation lines. Selected by Phase 10.7A (ADR 0089 §7). **Class 1: it acquired nothing.** Both writability facts are already in evidence on every successful session, and the repository measured that they are independent — on a real standby `in_hot_standby` was `on` while `default_transaction_read_only` was `off`, so *neither alone answers “can I write here”*. Only the first is rendered, so a session on a primary whose role or database carries `ALTER ROLE … SET default_transaction_read_only = on` shows `recovery: not in recovery` and nothing else. **ADR 0040 §20 is upheld, not reopened**: no rule reads either attribute, no `FindingCode` is added, and the change is at the renderer layer ADR 0085 §4 already authorized | DOE-014 … DOE-020. Binding: closed two-value render, absence drops the line and is never defaulted, subject is **the session and never the server**, the two facts stay distinct in strength, and **a real fixture must produce `default_transaction_read_only = on`** — one `-c` flag on a compose service. 10.7B may not close without it |
| **Kafka partition/leader/ISR availability** | **Rejected by Phase 10.7A (ADR 0089 §3.1), and the proof is one line.** `internal/adapter/kafka/wire/metadata.go:116` sets `request.Topics = []kmsg.MetadataRequestTopic{}` — **empty, not nil** — so the response carries no partition data at all. It is Class 3, and the minimum request that would yield partition state for an unknown application is *every topic*: an unbounded response, a fan-out multiplier across targets, and topic names — tenant identifiers — entering a shareable report | A record that authorizes a **topic-scoped** `Metadata` request and answers the data-minimization, fan-out and topic-name-redaction questions. Never as a side effect of a diagnosis phase |
| **Redis/Valkey and RabbitMQ/LavinMQ diagnostic intelligence** | **Audited by Phase 10.6A and neither selected (ADR 0088). Zero candidates admitted of eighteen weighed.** The framing was wrong: `docs/design/DIAGNOSTIC_INTELLIGENCE.md` §P's **10.4** row — *authn vs authz, vhost, protocol identity under timeout* — shipped in Phases 7.5–7.7 and 8.2, and **20 of the tree's 65 codes are already these two services'**. The slot is closed by delivery, not deferral. **RabbitMQ has no gap**: the vhost claim named as potentially high-value is already `RABBITMQ_VHOST_NOT_FOUND` + `RABBITMQ_VHOST_ACCESS_REFUSED`, and everything still measured is frozen *“may not be said”* by ADR 0069 §8 for want of an operator-supplied expectation. **Redis has one candidate and no producer**: lifting `LOADING`/`MASTERDOWN`/`BUSY` out of `REDIS_ENDPOINT_NOT_SERVING`'s prose — zero-cost, direct authority, the Phase 10.3 `53300` precedent — except **no fixture has ever observed any of the three**. Its role and mode facts have been terminal observation lines since Phase 7, and a finding over either fails the build (`TestReplicaRoleProducesNoFinding`, `TestClusterModeProducesNoFinding`) | **A measurement, not an argument.** Either a fixture that produces `LOADING`, `MASTERDOWN` or `BUSY`, or a multi-address run whose addresses disagree — every Redis, Valkey, RabbitMQ and LavinMQ fixture targets `127.0.0.1`, which under ADR 0059 yields one path, so the completeness-and-contrast aggregate Kafka and PostgreSQL both use has never been producible here (`docs/validation/PHASE106A_DIAGNOSTIC_OPPORTUNITY_AUDIT.md` §5) |
| **ADR 0069 §9 condition 3 — superseded in part by ADR 0088 §6.1** | **Found by Phase 10.6A and closed as a documentation-authority question; no code changed.** That condition invites migrating Redis client-limit exhaustion onto `RESOURCE_LIMIT_REACHED` as *“a separate, allowed correctness fix”*. ADR 0066 measured that `-ERR max number of clients reached` carries a **bare `ERR` prefix**, records that v1 therefore classifies it generically, and accepts the loss in as many words — *“inventing a distinction the prefix does not carry is the error this record exists to prevent”*. So the migration cannot be done without reading peer text, which ADR 0066 forbids outright. **The clause was normative** — it sits in a numbered *Reopen conditions* list and says *“allowed … correctness fix”* — and no Accepted record superseded it, so **ADR 0088 §6.1 does, in part**, and ADR 0069 carries a forward marker at the clause and in its header. Its factual premise was also imprecise: an `ERR` at the `HELLO` step is `Unsupported()` → `UNKNOWN`, and `diagnosis/redis.Hello` fires only on `FAIL`, so it yields **no finding**, where `REDIS_ENDPOINT_NOT_SERVING` needs an error at `PING`. Authoritative rule (RRI-017): **Redis `RESOURCE_LIMIT_REACHED` may be emitted only when the wire evidence structurally and authoritatively identifies a resource limit; a generic `ERR` plus server prose is insufficient, so client-limit exhaustion stays unclassified** | **Re-gated, not closed** (RRI-017a). A **closed, structurally authoritative signal**: a future Redis or Valkey release giving client-limit exhaustion its own prefix — ADR 0066 already calls an additional prefix *“additive and testable”* — or a protocol/probe expansion decided in its own record. **Never by text matching** |
| **`internal/service/rabbitmq/vocabulary.go` claims a build guard that does not exist** | **Found by Phase 10.6A; recorded, not fixed — 10.6A changes no Go and no test.** Of the six `channel_max`/`frame_max`/`heartbeat` attributes the comment says *“no rule reads them and a guard fails the build if one does (ADR 0069 §8)”*. The first half is true and was verified by reading every rule; **the second half is not** — no test in the tree scans the RabbitMQ diagnosis package for those reads. The same applies to `rabbitmq.anonymous_offered`, whose comment says *“observation only, permanently”* with no mechanical backing. **The invariant is real and currently maintained by review; the comment overstates enforcement**, so a future author would reasonably believe a violation cannot land silently — it can (RRI-016) | **A maintenance task, and Phase 10.6A deliberately neither chose nor implemented a closure.** Two legitimate shapes: **(A)** add the missing AST scan over `internal/diagnosis/rabbitmq` for the seven attribute identifiers, in the form `test/security` already uses for the SASL core and the diagnostic core; or **(B)** correct the comment to say the property is maintained by review, if mechanical enforcement is not intended. One small change in its own commit (ADR 0088 §6.2) |
| **Sanitising renderer observation values** | **Opened by Phase 10.3 and deliberately not fixed there.** `internal/render/terminal`'s `observations` mechanism prints endpoint-reported facts verbatim, and several of those fields are unbounded peer-controlled text: PostgreSQL's `wire.SessionParameters` allowlists four *keys* and retains each value as the server's own string with no length or character bound, and Redis and RabbitMQ already render verbatim `version` and `product` strings today. So a hostile endpoint can put escape sequences, newlines or megabytes on an operator's terminal through an observation line. Phase 10.3 declined to widen the surface while the question is open: PostgreSQL's one new line, `recovery`, uses a **closed two-value render function**, and `server_version` was deliberately not added as a second (ADR 0085 §4.4) | A renderer phase that decides once, for every service, what an observation value may contain and who enforces it — the wire boundary, the attribute type, or the renderer. It is one decision, not four |
| **Iterative diagnosis — measure, reason, measure again** | **Deferred by ADR 0078 §2.6 and re-affirmed by ADR 0086 §2.7 after Phase 10.4A weighed it.** Diagnosis is a pure function of frozen evidence, and that purity is what makes every existing guarantee cheap: a rule cannot dial, leak, escalate or be non-deterministic. `SelfCollectable: true` is forward-compatible with an iterative mode and authorizes nothing today | Its own ADR, and it must add all five: a report type describing a **search** with its own schema version (as ADR 0074 gave the aggregate one); a round budget nested in the execution budget with a truthful `Incomplete()`; an explicit operator opt-in, never a default; re-collection in the **collection** layer handing diagnosis a new frozen graph; and a security review of whether a later round may present a credential — presumed **no** |
| **Indistinguishable-set derivation, its identity mechanism, a set object, a set finding code, and any next-evidence ranking** | **All deferred to Phase 10.4C by ADR 0086 §2.0, §2.2a, §2.3 and §2.11, on measurement rather than taste.** Phase 10.4A counted producers: two `HYPOTHESIS` codes in the tree, both Kafka, **non-competing**; `BasisBuilder.Miss`, `.Block` and `.Contradict` have **zero** producers; `AuthorityCompleteContrast` has zero. A set engine would be built over an empty input, which ADR 0054 refuses as *owner before producer* — so **Phase 10.4B is explicitly forbidden to build one**, and NBE-044 fails the build if a generic grouping abstraction appears. The identity mechanism is deferred too: byte-equal `Discriminator` is the **minimum safe candidate, not a frozen rule**, because `Discriminator` is human-facing prose and a prose grouping key lets a wording edit change diagnostic behaviour — the coupling Phase 10.2A removed. Fuzzy matching stays forbidden permanently; `DiscriminatorID` is deliberately not added now | **A producer first**: a service phase that measures a genuine pair of mutually competing explanations settled by one observation. Phase 10.4C then decides ADR 0086 §2.2a against that real pair — exact equality (A), a typed key (B), or the missing `domain.Step` (C) — and builds a derivation only if the evidence shows one is needed. **No service is nominated**; Kafka SASL mechanism negotiation is a *candidate* the survey noticed, not a plan, and it may well dissolve the way `53300` did |
| **A typed credential-capability category on next-best evidence** | **Refused permanently by ADR 0086 §2.6.** A machine-readable *"svcdoctor could take this observation if you gave it a credential it lacks"* is a privilege-escalation prompt embedded in a shareable document: it invites an operator to widen a grant, invites a future phase to consume the field and ask for one, and puts the shape of a missing privilege into a document meant to be shared. The honest form is a `rationale` sentence, which stays | A security review that answers how it is not a privilege-escalation prompt. Not a diagnostic phase's decision |
| **`BasisBuilder.Miss`, `.Block` and `.Contradict` have no production producer** | Phase 10.0 froze four evidence relations; Phase 10.4A measured that only `.Support` is ever called, from **one** site. **Phase 10.5A asked the question a count cannot answer — do existing rules already compute these facts and flatten them into SUPPORT or prose? — audited all 22 production rules, and admitted none of 14 candidates (ADR 0087).** Each relation is latent for its own reason, and every reason is that the rule already uses the representation ADR 0081 §2.4 chose: contradiction fires as **suppression before a basis exists**, so recording it would mean emitting the finding the rule just declined to emit; every candidate "missing" observation is an `UNKNOWN` node *with an identifier*, which is ADR 0086 §2.1 **position 4**, while `Miss` takes a `domain.Step` precisely because a missing observation has none; and the one place a rule computes graph blocking and discards it — `Boundary.Blocked()` in `DIAG_FAILURE_BOUNDARY` — would gain a writer and no reader, for a fact the report already serializes on the cited node's `blockedBy` edge. **Neither 10.4B nor 10.5A forces a producer**: manufacturing one to justify a primitive is the inversion ADR 0054 forbids | A rule that genuinely records one, in the phase that needs it, **plus a reader in the same change-set** (ADR 0087 §2.10 item 14). One condition per relation is in `docs/validation/PHASE105A_EVIDENCE_RELATION_AUDIT.md` §6; the likeliest opener is a service phase reasoning about a stage svcdoctor never attempts, which is the only shape producing an observation with no evidence node |
| **`EvidenceBasis.supporting` and `Finding.EvidenceRefs` admit different sets; ADR 0086 §2.1 row 1 said they do not and is superseded** | **Found and recorded by Phase 10.5A; the documentation authority is closed, the code is not touched.** `BasisBuilder.Freeze` check 3 refuses a graph-blocked node in the supporting set unconditionally. **Three production findings cite one, and all three are right to**: `KAFKA_CREDENTIAL_WITHHELD`, `POSTGRES_CREDENTIAL_WITHHELD` and `POSTGRES_ADMISSION_SCOPE` each make a claim whose *subject* is a step that did not run, which is the other half of `docs/FINDINGS.md` §3.1 rule 11 and is now its rule 20. So `EvidenceRefs = basis.Supporting()` is a valid projection for a rule claiming something about the subject's condition and **is not a general law**; ADR 0087 §2.5 **supersedes** that cell, and ADR 0086 carries a forward marker at the row and in its header. The containment runs one way: `basis.Supporting()` ⊆ `EvidenceRefs`, never the reverse. Nothing was changed, because narrowing a check that implements ADR 0081 §2.4's sharpest case, with no rule waiting to use the relaxation, is the same inversion | The phase that must express such a claim *through a basis*. It decides which half moves — check 3 narrows, or those three findings change what they cite — and writes the reasoning down, not just the diff (REL-009, REL-012) |
| **Two guards in `AdmitConfidence` are vacuous, and they must be armed together** | **Found by Phase 10.5A.** The check that refuses `AuthorityCompleteContrast` beside a missing observation — *"I excluded every alternative" and "I could not look" are different claims* — reads `basis.missing`, and `Miss` has **zero** producers. It is safe today only because `AuthorityCompleteContrast` also has zero, so a false complete-contrast declaration is currently unreachable rather than caught. The contradiction cap is vacuous for the same reason. Separately, **the ladder has no ground admitting `HIGH` for a claim that restates a measurement svcdoctor took directly**: 21 of 22 rules set `ConfidenceHigh` as a literal, and `DIAG_FAILURE_BOUNDARY` routed through the ladder truthfully is `AuthorityNone` + 2 supporting nodes = `MEDIUM`. **Adopting the ladder is therefore not output-neutral** | REL-014 binds any phase that gives either primitive a producer: both, in one change-set. A third `Authority` needs a rule that requires `HIGH` and can declare neither existing ground, and it reopens ADR 0081 §2.3 in that phase's own record — never as a side effect (REL-015) |
| **NBE-021: one hypothesis asks an open question with no structured observation beside it** | **Accepted debt, recorded rather than paid, and non-blocking for Phase 10.4B.** ADR 0082 §2.5 requires a `HYPOTHESIS` carrying a `Discriminator` to also carry a `NEXT_EVIDENCE` recommendation. Exactly one production code breaks it: **`KAFKA_ADVERTISED_ENDPOINT_UNREACHABLE`** in its incomplete branch, whose three recommendations are the per-layer transport sentences of ADR 0034 §18 — written before the advice vocabulary existed and still unclassified. Phase 10.4B carried the vocabulary and **deliberately reclassified nothing**: classifying a service's advice is that service's judgement about its own claims, not generic plumbing (ADR 0086 §2.0). Enforcement is in place and is **shrink-only**: `test/diagnosis/nextevidenceinvariant_test.go` holds the inventory with the reason and pins its size at **1**, so a second exception cannot arrive without a deliberate edit; and `internal/diagnosis/kafka/nextevidencedebt_test.go` fails if the recorded shape stops being reachable, so a paid debt cannot linger | A **Kafka** phase that decides the kind and safety class of its three transport recommendations, on Kafka's own judgement. Closing it means deleting both the inventory entry and the kafka pin in the same change |
| **`internal/cli.TestOversizeErrorsCarryNoSecretDerivedValue` is flaky on macOS** | **A test false positive, not a production leak, and deliberately not fixed in Phase 10.4B.** The test asserts that an oversize-credential error carries no secret-derived byte count, and scans the **whole** error string for `8192`, `8211`, `12288` and `bytes read`. The message embeds the `t.TempDir()` path, and a macOS `/var/folders/…` component can contain one of those digit strings by coincidence — measured at roughly 92 collisions per 200,000 synthetic paths. Observed once during Phase 10.4B, then 40/40 green in isolation and clean across two consecutive full runs. **Phase 10.4B modifies nothing in `internal/cli`**, and the security assertion it makes is real and must not be weakened | Narrow the scan to the meaningful error payload — exclude or structurally separate the known temporary path — while keeping the assertion over everything the operator actually sees. A one-line test-scope fix in its own change, never by relaxing the canary list |
...
| A third service produces a capacity ceiling. **Re-gated by ADR 0088 §6.1: Redis `max number of clients` is *not* the candidate** — it carries a bare `ERR` and ADR 0066 forbids reading the text that would distinguish it. Reopens on a **closed, structurally authoritative signal** | ADR 0069 §9, **superseded in part by ADR 0088 §6.1** |
...
| **Recommendation kind and safety class live in `internal/diagnosis`, not on `domain.Recommendation`.** ADR 0082 §2.1 puts them on the report type, additively; adding a field would change the JSON of every report carrying a recommendation | ~~Closes in Phase 10.1b.~~ **It did not.** Phases 10.1b, 10.2 and 10.3 each declined to move them, and Phase 10.4A measured the cost: both service packages build a fully classified `diagnosis.Advice`, run every guardrail over it, and then discard `kind`, `safety`, `rationale` and `selfCollectable`, so a report cannot say that a suggestion is an observation rather than a change. **Now the substance of Phase 10.4B** (ADR 0086 §2.8). The field count is **four**, not three — ADR 0083 §2.1 undercounted by missing ADR 0082 §2.4's `SelfCollectable` |
