...
// The Redis BASIC validation harness.
//
// # It drives the product, not a shortcut
//
// Every scenario calls app.DiagnoseRedis — the same entry point
// `svcdoctor diagnose redis` calls. A harness that sequenced the adapter itself
...
//
// # Ground truth comes from redis-cli, before svcdoctor is asked
//
// Each scenario that asserts something about the server establishes it
// independently first, through `docker exec ... redis-cli`. That ordering is the
...
const (
	portBaseline = 56379 // no auth, plaintext
	portPassword = 56380 // requirepass
	portACL      = 56381 // the ACL matrix
	portTLS      = 56382 // TLS, tls-auth-clients no
	portMTLS     = 56383 // TLS, tls-auth-clients yes (the Redis default)
	portReplica  = 56384
	portCluster  = 56385
	portSentinel = 56386
)
...
// command is redis-cli, which is not svcdoctor and shares no code with it.
func groundTruth(t *testing.T, container string, args ...string) string {
	t.Helper()
	full := append([]string{"exec", container, "redis-cli"}, args...)
...
func assertNoKeyspaceCommandReached(t *testing.T, container string) {
	...
	if container == "svcd-redis-cluster" || container == "svcd-valkey-cluster" {
